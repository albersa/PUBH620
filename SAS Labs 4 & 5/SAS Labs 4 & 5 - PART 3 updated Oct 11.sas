...
title1 "UNADJUSTED HPV Vaccine Series Initiation Prevalence";
...
data pubh620.mt_nisteen17;
	...
	where STATE=30;  /*?  What number do we need to use here?  Look at formats code!*/
run;

data pubh620.test_nisteen17;
	set PUBH620.nisteenpuf17_P;
	where STATE=30 OR STATE=13 OR STATE=2 ;  /*?  What number do we need to use here?  Look at formats code!*/
run;

data pubh620.test2_nisteen17;
	set PUBH620.nisteenpuf17_P;
	where STATE in (2, 13, 30); 
run;
...
*If time, show how to do multiple states;
...
*don't have to use surveyfreq to check variables, can use just proc freq;

proc freq data=pubh620.mt_nisteen17;
tables raceethk * race_eth / list;
run;
...
*Hypotheses for chisquare
H0 = There is not an association between race/ethnicity and UTDHPV (yes/no)
HA= There IS an association between race/ethnicity and UTDHPV (yes/no);

*The result was p=0.77, is this significant or not?
a signifcant result would be a p<0.05;
*So, because we have a nonsignificant p-value (0.77), then we do not have evidence to reject the null (H0), 
therefore, we would say there is not an association between r/e and UTDHPV;
...
*Our hypotheses:
*H0 = There is not a difference in age between groups (HPV UTD or not)
Ha= There IS a difference in age between groups (HPV UTD or not);

*We then look at the results from AGE in the Estimated Regression Coefficients
table. Since the p-value (shown as Pr>[t]) is 0.1768, we do not have enough
evidence to reject the null, and we conclude age was not
